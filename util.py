...
def compute_class_weights(labels):
    ...
    #class_weights[:2] += 0.1
    #class_weights[2:3] += 0.2
    return torch.tensor(class_weights, dtype=torch.float32)
# 计算类别权重
def compute_class_weights1(labels):
    # 确保 labels 是一维数组
    labels = labels.flatten() if len(labels.shape) > 1 else labels
    ...
    # 计算每个类别的权重 = 每个类别点数 / 总点数
    class_weights = class_counts / total_count

    # 计算最终的 class_weights = 1 / (权重 + 0.02)
    adjusted_class_weights = 1 / (class_weights + 0.02)

    # 使用 torch 返回 tensor 格式
    # 注意 class_weights 顺序需要与 unique_classes 保持一致
    class_weights_tensor = torch.tensor(adjusted_class_weights, dtype=torch.float32)

    return class_weights_tensor

def cal_loss(pred, gold, weights=None, smoothing=False, ignore_index=255):
    ...
    if smoothing:
        ...
        loss = F.cross_entropy(
            pred, gold, weight=weights, reduction='mean',
            ignore_index=ignore_index)

    return loss

def my_cal_loss(pred, gold, points, threshold, lambda_z=0.1, weights=None):
    """
    计算损失函数，结合交叉熵损失和 z 坐标的相似性损失。

    参数:
    pred -- 预测标签，形状为 (batch_size * num_points, num_classes)
    gold -- 真实标签，形状为 (batch_size * num_points,)
    points -- 点的坐标，形状为 (batch_size, 6, num_points)
    lambda_z -- z 坐标损失的权重
    weights -- 类别权重，形状为 (num_classes,)

    返回:
    total_loss -- 结合交叉熵损失和 z 坐标相似性损失的总损失
    """
    # 计算交叉熵损失
    loss = F.cross_entropy(pred, gold, weight=weights, reduction='mean')

    # 提取 z 坐标
    z_coords = points[:, 2, :].reshape(-1).cpu().numpy()  # (batch_size * num_points, )
    # 使用 KD 树找到每个点的近邻
    kdtree = KDTree(z_coords.reshape(-1, 1))
    distances, indices = kdtree.query(z_coords.reshape(-1, 1), k=2)  # 只找一个最近邻
    # 将 z_coords 和 pred 移动到 CPU 上
    z_coords = torch.tensor(z_coords, device=points.device)
    pred = pred.argmax(dim=1).reshape(-1)  # 展平预测标签
    # 获取最近邻的预测标签
    neighbor_indices = indices[:, 1]  # 忽略自身
    neighbor_preds = pred[neighbor_indices]
    # 计算 z 坐标相近但标签不同的点对
    z_diff = (z_coords - z_coords[neighbor_indices]).abs()
    pred_diff = (pred != neighbor_preds).float()
    z_loss = torch.sum(pred_diff) / len(pred)

    '''
    kdtree = KDTree(z_coords)
    indices = kdtree.query_radius(z_coords, r=threshold)
    # 初始化相似性损失
    z_loss = 0.0
    num_comparisons = 0  # 记录比较次数
    # 对每个点，检查与其相似的点
    for i, neighbors in enumerate(indices):
        pred_i = pred[i].argmax().item()  # 当前点的预测标签
        for j in neighbors:
            if i != j:  # 确保不与自身比较
                num_comparisons += 1
                pred_j = pred[j].argmax().item()  # 相似点的预测标签
                if pred_i != pred_j:  # 如果标签不同
                    z_loss += 1.0  # 不同标签加损失
    # 计算平均 z_loss
    if num_comparisons > 0:
        z_loss /= num_comparisons  # 求平均
    else:
        z_loss = 0.0  # 如果没有进行比较，z_loss 设为 0'''
    
    # 结合交叉熵损失和 z 方向上的平滑性损失
    total_loss = loss + lambda_z * z_loss
    return total_loss
