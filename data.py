...
def download_modelnet40_win():
    ...
    if not os.path.exists(os.path.join(DATA_DIR, 'modelnet40_ply_hdf5_2048')):
        ...
        # 使用 Python 的内置库下载文件
        print(f"Downloading {zipfile_name} from {www}...")
        ...
        # 移动解压后的文件夹到 DATA_DIR
        extracted_folder = 'modelnet40_ply_hdf5_2048'
        os.system(f'move {extracted_folder} {DATA_DIR}')
...
def load_my_data(partition, exp_name):
    BASE_DIR = 'E:\\lcy\\dgcnn\\data\\coordinate_correction\\'
    DATA_DIR = os.path.join(BASE_DIR, exp_name)  # 你的 HDF5 文件所在的文件夹
    all_data = []
    all_label = []
    
    # 查找指定 partition 的 HDF5 文件
    for h5_name in glob.glob(os.path.join(DATA_DIR, '*%s*.h5'%partition)):
        print(f"Found HDF5 file: {h5_name}")  # 打印找到的文件名
...
def load_data_partseg(partition, exp_name):
...
class ModelNet(Dataset):
    def __init__(self, num_points, exp_name, partition='train'):
        self.data, self.seg = load_my_data(partition, exp_name)
    ...
    def __getitem__(self, item):
        ...
        #print(f"pointcloud shape: {pointcloud.shape}, seg shape: {seg.shape}")
...
class ShapeNetPart(Dataset):
    def __init__(self, num_points, partition='train', class_choice=None):
        self.data, self.seg = load_data_partseg(partition)
        self.cat2id = {'bridge': 0}
        self.seg_num = [6]
        self.index_start = [0]
        self.num_points = num_points
        self.partition = partition        
